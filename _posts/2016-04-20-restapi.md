# REST API
![](http://10.252.88.47/imgs/restapi.jpg)
------------------------------
...
Host명이 ims.com ~~사실은 김현진의 리눅스PC~~ 인 ims파트라는 Resource에 대해 예를 들면,

- `GET` [`http://ims.com/cells`](http://10.252.88.47:8081/cells)
 모든 셀 반환
- `GET` [`http://ims.com/cells/goCell`](http://10.252.88.47:8081/cells/goCell)
 고셀에 대한 정보 반환

- `POST` `http://ims.com/cells`
 새로운 셀 등록
 <form action="http://10.252.88.47:8081/cells" method="post">
<textarea id="idid" name="cellinfo" cols="85" rows="7"></textarea>
  <input type="submit" value="Submit (JSON 형식이 틀리면 무시)">
</form>
입력 예)
```json
{
...
- `DELETE` `http://ims.com/cells/goCell`
 고셀 삭제

- `PUT` `http://ims.com/cells/goCell?name=HakCell`
 고셀의 이름을 학셀로 변경

라고 할 수 있는데, 이처럼 URI와 Method만 보고도 어떤 작업이 이루어질지 예상가능 해야한다. (SELF-DESCRITIVENESS)
~~`DELETE` `PUT`은 데모를 생략한다~~
...
![](http://10.252.88.47/imgs/fboauth.png)
